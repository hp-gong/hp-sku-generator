  ...
  class HP_Simple_SKU{
	 ...
	 // Uninstall Function & Remove woo_sku tables from the databases
	  ...
	  public static function check_if_woo_install(){
	   ...
	   public static function check_versions(){
	    ...
	    require_once(ABSPATH . 'wp-admin/includes/plugin.php');
	    deactivate_plugins( plugin_basename( __FILE__ ));
	    wp_die( __('Simple SKU Generator is disabled.<br>Simple SKU Generator requires a minimum of WooCommerce v3.0.7.<br><a href="'.$url.'">Return to the Plugins section</a>'));
	    }
	    }
    ...
    if ($_SERVER['REQUEST_METHOD']== "POST"){
	 ...
	 function hp_sku_display(){
      ...
      echo '<div id="dvData">';
	  echo '<table id="sku_data" style="border-collapse: collapse; width: 40%; border: 1px solid black; background-color: white;">
	  <thead>
	  <tr>
	  ...
	  echo '<input name="remove_sku" value="'.sanitize_text_field($row3->w_id).'" type="hidden" />';
	  	  }
	  }
	  echo '<input type="hidden" id="filedata" name="data" value="">';
